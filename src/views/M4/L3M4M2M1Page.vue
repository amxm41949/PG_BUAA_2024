    ...
    <el-container>
      <el-header>
        ...
        <div class="header-content">
          ...
          <span class="text-large font-600 mr-3 title">L3M4M2M1(日志恢复模块)</span>
        </div>
...
export default {
  data() {
    return {
      active: 0,
      imageSrc: '/pictures/level3/L3M4M2M1.png', // 替换为您的图片路径
      hotspots: [
        {
          id: '1',
          shape: 'poly',
...
日志进行崩溃恢复时，将会首先调用位于\`xlog.c\`中的\`StartupXLog\`函数。\`StartupXLog()\` 是启动过程的主要入口点，该函数会在\`postmaster\`或独立后端启动期间被调用。它协调数据库的启动，执行WAL恢复，并从WAL恢复过渡到正常操作。`,
      md: new MarkdownIt({
