...
export default {
    data() {
        return {
            active: 0,
            imageSrc: '/pictures/level3/L2M3M3M4.png', // 替换为您的图片路径

            markdownText:
                `**总览**

操作系统对于单个进程中能打开的文件数是有限制的，而数据库需要频繁的操作大量的文件。postgreSQL 实现了 VFD机制。

本质上是在 postgreSQL 软件内，对操作系统的文件系统进行了又一层封装。这样所有需要操作文件的地方可以调用 VFD，VFD 分配不受操作系统数量限制，同时也隐藏了对操作系统管理的细节。

虚拟文件描述符 VFD、真实文件描述符 FD、磁盘文件之间的关系如左图所示。

总的来说，操作系统中，真实文件描述符 FD 机制实现了对磁盘文件的封装；postgreSQL 软件中，建立虚拟文件描述符 VFD 和真实文件描述符 FD 的映射，实现了对操作系统文件系统的又一次封装。
